...
(function(root){'use strict';
 ...
 for(let i=0;i<32;i++){const vertical=i%2===0,lane=(i%7+1)*400+(i%3?72:-72),travel=(i*179+time*(10+i%5))%(vertical?z.h:z.w),x=vertical?lane:travel,y=vertical?travel:lane;if(inside(x,y))A.human(c,x,y,.29,{time:time+i,walk:.65,face:i%3?1:-1,skin:i%3?'#cda180':'#865b48',color:['#ffcf96','#a2e1e7','#e3ace3','#aec499'][i%4]})}
 for(const p of places)if(inside(p.x,p.y)){if(p.type==='npc'){const a=C.allies[p.ally]||{};A.human(c,p.x,p.y,.43,{...a,time});A.text(c,p.name,p.x,p.y+22,'#eadbf1',11)}else{landmark(c,p,time);A.token(c,{...p,type:p.type==='campaign'?'story':p.type},time)}}
 if(goal){const route=streetRoute(zone,player,goal);c.setLineDash([8,12]);stroke(c,route.map(p=>[p.x,p.y]),'#ffdc8970',3);c.setLineDash([])}
 const people=party.map((id,i)=>{let pos=trail[Math.min(trail.length-1,(i+1)*10)]||player;if(Math.hypot(pos.x-player.x,pos.y-player.y)<25)pos=D.safePosition(player.x+(i-1)*34,player.y+44+i%2*20,zone);return{id,pos}}).concat([{id:'hero',pos:player}]);people.sort((a,b)=>a.pos.y-b.pos.y);for(const person of people){const a=C.allies[person.id]||{color:'#ffce8c',skin:'#ad7859',hair:'#282638'};A.human(c,person.pos.x,person.pos.y,.48,{...a,time,walk:player.walk,face:person.pos.face||player.face});A.text(c,person.id==='hero'?heroName:a.name,person.pos.x,person.pos.y+22,person.id==='hero'?'#ffe4a8':a.color,12)}
